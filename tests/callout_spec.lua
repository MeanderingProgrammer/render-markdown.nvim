...
describe('callout.md', function()
    it('default', function()
        ...
        local info = 'RmInfo'
        marks:add(row:inc(), row:get(), 0, 2, util.quote('%s ', info))
        marks:add(row:get(), row:get(), 2, 9, {
            virt_text = { { '󰋽 Note', info } },
            virt_text_pos = 'overlay',
        })
        marks:add(row:inc(), row:get(), 0, 1, util.quote('%s', info))
        marks:add(row:inc(), row:get(), 0, 2, util.quote('%s ', info))
        marks:add(row:inc(), row:get(), 0, 1, util.quote('%s', info))
        marks:add(row:inc(), row:get(), 0, 2, util.quote('%s ', info))
        ...
        local ok = 'RmSuccess'
        marks:add(row:inc(), row:get(), 0, 2, util.quote('%s ', ok))
        marks:add(row:get(), row:get(), 2, 8, {
            virt_text = { { '󰌶 Tip', ok } },
            virt_text_pos = 'overlay',
        })
        marks:add(row:inc(), row:get(), 0, 1, util.quote('%s', ok))
        marks:add(row:inc(), row:get(), 0, 2, util.quote('%s ', ok))
        ...
        marks:add(row:get(), row:get(), 0, 2, util.quote('%s ', ok))
        ...
        local hint = 'RmHint'
        marks:add(row:inc(), row:get(), 0, 2, util.quote('%s ', hint))
        ...
        local warn = 'RmWarn'
        marks:add(row:inc(), row:get(), 0, 2, util.quote('%s ', warn))
        ...
        local err = 'RmError'
        marks:add(row:inc(), row:get(), 0, 2, util.quote('%s ', err))
        marks:add(row:get(), row:get(), 2, 12, {
